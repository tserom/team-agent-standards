...
开分支 / 动手改代码前：若主工作区有未提交改动，先按 [`dirty-workspace-worktree.md`](dirty-workspace-worktree.md) 判断——干净优先主工作区；**仅**调试 proxy（如 `config/proxy.ts`）可 discard/stash；其它脏则用 worktree，禁止擅自 stash/discard。
