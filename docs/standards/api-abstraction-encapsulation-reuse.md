...
### 3.4 响应字段契约（跳转 / 拼参 / L0 业务解析）

**适用**：L3 拼路由、L0 从行对象取 id/单号、OpenSpec `API_PENDING` 字段约定。

| # | 问题 | 是 → | 否 → |
|---|------|------|------|
| 1 | 字段是否在本接口联调 VO / `@ApiModelProperty` 中**已声明**？ | 写入 `*Row` 类型，跳转/解析**只读该字段** | **禁止**用 `??` 猜其它接口/参考页字段 |
| 2 | 是否仅为 BsSula 响应包裹或分页 `total` 兜底？ | L0 注释标明「框架形态」 | 不得与业务字段混在同一 `??` 链 |
| 3 | 是否展示列字典反查（如 `statusName` 空 → 字典）？ | 仅 render；**不得**用于跳转 | — |
| 4 | 业务上必须回退（如接口暂缺 id）？ | 登记 **回退表**（见下）；MR/Agent 回复**单列**待确认 | 禁止静默回退 |

**回退登记表**（`API_PENDING.md` 或 Skill `LOCAL.md`）列：

| 位置 | 回退内容 | 性质（框架 / 宿主 / 业务缺口 / 展示） | 待确认 |
|------|----------|----------------------------------------|--------|

**反例**：无名包裹新接口仅有 `id` + `inRecordCode`，却复制 other-in 的 `recordId ?? noticeRecordId ?? id` — 后续 VO 多返回 `recordId` 时会**静默走错跳转**，难以排查。

**正例**：`ReturnPackageReceiptBatchRow` + `go*ReceiptNotice(record)` 只解构 `id`、`inRecordCode`；结果单仅拆 `resultRecordCode`。
...
6. **禁止**跳转/拼参使用**未在联调文档或 `*Row` 类型声明**的业务字段 `??` 链；必须回退时未登记回退表。
...
- [ ] 跳转/拼参是否仅 `*Row` 声明字段？业务回退是否已登记且待确认项已列出？
```
...
| 2026-06-01 | 1.1 | §3.4 响应字段契约；禁止未声明业务字段回退链；CR 清单增补 |
